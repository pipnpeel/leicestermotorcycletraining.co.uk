			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				if(!empty($src))
				{
					...
					else
					{
						...
						if($link)
						{
							$output.= "<a href='{$link}' class='avia_image ".$meta['el_class']." ".$this->class_by_arguments('align' ,$atts, true)."' {$linktarget}><img class='avia_image {$class}' src='{$src}' alt='{$alt}' title='{$title}' $markup_url /></a>";
						}
						else
						{
