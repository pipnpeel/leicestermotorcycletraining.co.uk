...
if ( !class_exists( 'avia_partner_logo' ) )
{
	class avia_partner_logo
	{
		...
		public function html()
		{
            ...
            $thumb_fallback = "";
            $output .= "<div {$data} class='avia-logo-element-container avia-logo-{$type} avia-content-slider avia-smallarrow-slider avia-content-{$type}-active avia-content-slider".avia_partner_logo::$slider." avia-content-slider-{$total} {$class}' >";
                ...
                foreach($this->id_array as $key => $id)
                {
                    ...
                    $output .= !empty($link) ? "<a href='{$link}' data-rel='slide-".avia_partner_logo::$slider."' class='slide-image' title='{$linktitle}' {$link_target} >{$img}</a>" : $img;
                    $output .= "</div>";
